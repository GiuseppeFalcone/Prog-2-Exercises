...
public class BottigliaConTappo extends Bottiglia {
    ...
    public BottigliaConTappo(int capacita) {
        /* invochiamo il costruttore della classe superiore per fare le inizializzazioni della capacita' */
        super(capacita);
        // supponiamo che la bottiglia sia inizialmente chiusa
        aperta = false;
    }
    /* La chiamata al costruttore della classe superiore deve essere la prima istruzione del costruttore della sottoclasse. Il costruttore della classe superiore può essere richiamato solo dal costruttore della sottoclasse. Se il costruttore della sottoclasse non richiama esplicitamente un costruttore della classe superiore, per prima cosa viene chiamato automaticamente il costruttore predefinito della classe superiore, quello senza parametri (super()); se la classe superiore non ha un costruttore senza parametri il compilatore genera un errore. */
...
// Ereditiamo i metodi get, set (ma non toString()) da Bottiglia.Bottiglia
    ...
    public int aggiungi(int quantita) {
        if (aperta)
            return super.aggiungi(quantita); /*super.aggiungi() indica il metodo "aggiungi" nella classe Bottiglia.Bottiglia che stiamo estendendo */
        else return 0;
    }
    ...
    public int rimuovi(int quantita) {
        if (aperta)
            return super.rimuovi(quantita); /*super.rimuovi() indica il metodo "rimuovi" nella classe Bottiglia.Bottiglia che stiamo estendendo */
        else return 0;
    }

    /* OVERRIDE del metodo "toString()". Alla stringa che descrive una bottiglia aggiungiamo l’informazione aperta/chiusa */
