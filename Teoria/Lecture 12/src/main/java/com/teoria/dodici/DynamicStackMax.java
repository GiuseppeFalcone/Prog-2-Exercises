...
public class DynamicStackMax extends DynamicStack {
    ...
    public int getMax() {
        assert !empty() : "List cannot be empty"; // se pila vuota: non corretto chiedere il massimo
        return max;
    }
...
// alla pila aggiornando il valore del massimo
    ...
    private void resetMax() {
        ...
        * Oppure
        * max = Integer.MIN_VALUE
        * for (Note cursor = top; cursor != null; cursor.getNext())
        *       max = Math.max(max, cursor.getElem());
        *
        */
    }
