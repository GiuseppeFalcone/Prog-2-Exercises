package com.teoria.quattordici.MiniLinkedList;//Node.java Riutilizziamo la classe Node della Lezione 08
...
public class MiniLinkedList {
    ...
    /* Definiamo un metodo che inserisce una MiniLinkedList in un oggetto che fa parte della classe MiniIterator. MiniIterator consente di eseguire un ciclo senza rendere pubblici gli indirizzi dei nodi. */
    public MiniIterator generateIterator() {
...
/** DEFINIZIONE get(i),set(i,x),add(i,x),remove(i) usando node(i) */
    ...
    /**
     * cerca l'elemento toReplace e se lo trova inserisce nell'elemento newValue
     *
     * @param toReplace vecchio valore
     * @param newValue  nuovo valore
     * @return true se l'element toReplpace è stato trovato
     */
    public boolean replaceValue(int toReplace, int newValue) {
        MiniIterator iterator = generateIterator();
        while (generateIterator().hasNext()) {
            if (iterator.isNextValue(toReplace)) {
                iterator.assignNextValue(newValue);
                return true;
            }
